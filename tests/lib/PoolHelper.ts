...
import { Dictionary, Sender, toNano } from "@ton/core";
...
export async function withdrawFee(sender: Sender, gas: number, feeReceiver: SandboxContract<TreasuryContract>) {
    let balanceBefore = await getAllBalance();

    const trxResult = await TestEnv.pool.send(
        sender, { value: toNano(gas) }, { $$type: 'WithdrawFee', feeReceiver: feeReceiver.address }
    );
...
export async function increaseAUM(sender: Sender, amount: number, gas?: number) {
    let balanceBefore = await getAllBalance();

    const trxResult =  await TestEnv.pool.send(
        sender,
        {
            value: toNano(amount + (gas || 0.2)),
        },
        {
            $$type: 'IncreaseAum',
            trxId: 1n,
    ...
    const trxResult = await TestEnv.pool.send(
        ...
        },
        {
            ...
            gasConfig: {
                ...
                updateConfigGas: toNano(0.05),
                withdrawFeeGas: toNano(0.05),
