...
import { toJettonUnits, toPriceUnits, updateJettonContent } from './lib/TokenHelper';
import { withdrawFee, feedPrices, updateBaseConfig } from './lib/PoolHelper';
import { createIncreaseLiquidityOrder, executeLiquidityOrder } from './lib/LPHelper';
import { createCompensate, executeCompensate } from './lib/OrderBookHelper';
...
describe('pool', () => {
    ...
    it('should correctly claim fees', async () => {
        ...
        const claimResult = await withdrawFee(sender.getSender(), 0.2, feeReceiver);
        ...
        expect((await TestEnv.pool.getPoolStat()).feeReserve).toEqual(toJettonUnits(0));
    });
    ...
    it('should revert claim fees if gas is not enough', async () => {
        const sender = TestEnv.user0;
        const feeReceiver = TestEnv.user0;
        const claimResult = await withdrawFee(sender.getSender(), 0.01, feeReceiver);
