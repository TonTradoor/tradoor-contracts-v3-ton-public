...
import { disbaleToken, increaseAUM, stopContract } from './lib/PoolHelper';
...
describe('PERP', () => {
    ...
    it('should revert perp order if insufficient gas is provided.', async () => {
        ...
        const gas = 0.02; // insufficient gas
        // create order
        const createResult = await createIncreasePerpOrder(user0, executionFee, isMarket, tokenId, isLong, margin, size, triggerPrice, tpSize, tpPrice, slSize, slPrice, gas);
        expect(createResult.trxResult.transactions).toHaveTransaction({
            from: user0.address,
            to: pool.address,
            success: false
    ...
    it('should revert increase AUM if original caller is not the manager address.', async () => {
        const result = await increaseAUM(user0.getSender(), 1000);
    ...
    it('should revert cancel perp order if the order is pending.', async () => {
    ...
    it('should revert execute perp order if order is pending.', async () => {
    ...
    it('should revert execute perp order if margin rate is too high when decreasing the perpetual position.', async () => {
    ...
    it('should revert execute perp order if leverage is too high after decreasing the perpetual position.', async () => {
    ...
    it('should revert adl perp position if the margin is too high.', async () => {
