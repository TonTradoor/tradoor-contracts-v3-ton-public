...
function initTLPJettonWallet_init_args(src: TLPJettonWallet_init_args) {
    return (builder: Builder) => {
        let b_0 = builder;
        b_0.storeAddress(src.owner);
        b_0.storeAddress(src.jetton_master);
...
const TLPJettonWallet_types: ABIType[] = [
...
export class TLPJettonWallet implements Contract {
    
    static async init(owner: Address, jetton_master: Address) {
        return await TLPJettonWallet_init(owner, jetton_master);
    }
    
    static async fromInit(owner: Address, jetton_master: Address) {
        const init = await TLPJettonWallet_init(owner, jetton_master);
        const address = contractAddress(0, init);
        return new TLPJettonWallet(address, init);
    }
    
    static fromAddress(address: Address) {
        return new TLPJettonWallet(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  TLPJettonWallet_types,
        getters: TLPJettonWallet_getters,
        receivers: TLPJettonWallet_receivers,
        errors: TLPJettonWallet_errors,
    };
