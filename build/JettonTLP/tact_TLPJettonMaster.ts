...
function dictValueParserTLPJettonMaster$Data(): DictionaryValue<TLPJettonMaster$Data> {
 ...
 type TLPJettonMaster_init_args = {
    $$type: 'TLPJettonMaster_init_args';
    owner: Address;
    jetton_content: Cell;
}

function initTLPJettonMaster_init_args(src: TLPJettonMaster_init_args) {
    return (builder: Builder) => {
        let b_0 = builder;
        b_0.storeAddress(src.owner);
        b_0.storeRef(src.jetton_content);
...
async function TLPJettonMaster_init(owner: Address, jetton_content: Cell) {
    ...
    initTLPJettonMaster_init_args({ $$type: 'TLPJettonMaster_init_args', owner, jetton_content })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

const TLPJettonMaster_errors: { [key: number]: { message: string } } = {
...
const TLPJettonMaster_getters: ABIGetter[] = [
...
export const TLPJettonMaster_getterMapping: { [key: string]: string } = {
    'get_jetton_data': 'getGetJettonData',
    'get_wallet_address': 'getGetWalletAddress',
    'owner': 'getOwner',
}

const TLPJettonMaster_receivers: ABIReceiver[] = [
...
export class TLPJettonMaster implements Contract {
    
    static async init(owner: Address, jetton_content: Cell) {
        return await TLPJettonMaster_init(owner, jetton_content);
    }
    
    static async fromInit(owner: Address, jetton_content: Cell) {
        const init = await TLPJettonMaster_init(owner, jetton_content);
        const address = contractAddress(0, init);
        return new TLPJettonMaster(address, init);
    }
    
    static fromAddress(address: Address) {
        return new TLPJettonMaster(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  TLPJettonMaster_types,
        getters: TLPJettonMaster_getters,
        receivers: TLPJettonMaster_receivers,
        errors: TLPJettonMaster_errors,
    };
