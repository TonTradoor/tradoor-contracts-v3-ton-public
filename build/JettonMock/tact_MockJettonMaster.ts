...
function initMockJettonMaster_init_args(src: MockJettonMaster_init_args) {
    return (builder: Builder) => {
        let b_0 = builder;
        b_0.storeAddress(src.owner);
        b_0.storeRef(src.jetton_content);
...
const MockJettonMaster_types: ABIType[] = [
...
export const MockJettonMaster_getterMapping: { [key: string]: string } = {
    'get_jetton_data': 'getGetJettonData',
    'get_wallet_address': 'getGetWalletAddress',
    'owner': 'getOwner',
}

const MockJettonMaster_receivers: ABIReceiver[] = [
...
export class MockJettonMaster implements Contract {
    
    static async init(owner: Address, jetton_content: Cell) {
        return await MockJettonMaster_init(owner, jetton_content);
    }
    
    static async fromInit(owner: Address, jetton_content: Cell) {
        const init = await MockJettonMaster_init(owner, jetton_content);
        const address = contractAddress(0, init);
        return new MockJettonMaster(address, init);
    }
    
    static fromAddress(address: Address) {
        return new MockJettonMaster(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  MockJettonMaster_types,
        getters: MockJettonMaster_getters,
        receivers: MockJettonMaster_receivers,
        errors: MockJettonMaster_errors,
    };
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: 'Owner: MintClose' | JettonUpdateContent | JettonBurnNotification | JettonMint | Deploy) {
