...
function dictValueParserMockJettonMaster$Data(): DictionaryValue<MockJettonMaster$Data> {
 ...
 type MockJettonWallet_init_args = {
    $$type: 'MockJettonWallet_init_args';
    owner: Address;
    jetton_master: Address;
}

function initMockJettonWallet_init_args(src: MockJettonWallet_init_args) {
    return (builder: Builder) => {
        let b_0 = builder;
        b_0.storeAddress(src.owner);
        b_0.storeAddress(src.jetton_master);
...
const MockJettonWallet_types: ABIType[] = [
...
const MockJettonWallet_getters: ABIGetter[] = [
    {"name":"get_wallet_data","arguments":[],"returnType":{"kind":"simple","type":"WalletData","optional":false}},
]

export const MockJettonWallet_getterMapping: { [key: string]: string } = {
    'get_wallet_data': 'getGetWalletData',
}

const MockJettonWallet_receivers: ABIReceiver[] = [
...
export class MockJettonWallet implements Contract {
    ...
    static async init(owner: Address, jetton_master: Address) {
        return await MockJettonWallet_init(owner, jetton_master);
    }
    
    static async fromInit(owner: Address, jetton_master: Address) {
        const init = await MockJettonWallet_init(owner, jetton_master);
        const address = contractAddress(0, init);
        return new MockJettonWallet(address, init);
    }
    ...
    readonly abi: ContractABI = {
        types:  MockJettonWallet_types,
        getters: MockJettonWallet_getters,
        receivers: MockJettonWallet_receivers,
